        ...
        private const val CSS_JS_HACK = """
            ...
            window.addCSSJSHack=function(){
                ...
                cssint=setInterval(function(){
                    ...
                    window._initRs&&_initRs();
                },5000);
            };
            window._initRs=function(){
                if(document.getElementById('spl-rs'))return;
                var nav=document.querySelector('nav');
                if(!nav)return;
                var bar=document.querySelector('.LayoutResizer__resize-bar');
                if(!bar)return;
                var ct=bar.closest('[style*="--left-sidebar-width"]')||document.querySelector('[style*="--left-sidebar-width"]');
                if(!ct)return;
